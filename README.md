...
Books are displayed as a responsive grid of cards. Cover images and descriptions were fetched from the Google Books API during data preparation using a personal API key stored securely as a Streamlit secret. The results were saved into items_api_info.csv alongside the rest of the book metadata. Live API calls at render time were attempted during development and worked correctly in the local environment, but consistently failed after deploying to Streamlit Cloud. The reason for that is likely that Streamlit Cloud routes all outbound traffic through a shared pool of IP addresses used by every app hosted on the platform. Google's API infrastructure detects requests coming from these addresses as automated traffic and either returns that there are too many requests or drops the request, even if the API key itself is valid. The pre-fetched CSV is therefore used as the data source for covers and descriptions in the deployed version. We kept the code used to fetch information with the Google Books API, but commented it out. When no cover is available from the API, which is frequent for older or more niche academic titles in this library's collection, a custom SVG placeholder is generated. Each placeholder has a gradient background, a book icon, and the book title. The color scheme is assigned by hashing the book title, so the same book always renders the same cover style regardless of who views it or when. Descriptions fetched from the API are stripped of any HTML formatting and shown as a short text snippet beneath the subject tag, if this information is available. The subject itself is taken from items metadata. If the book has already been read, the book card is marked as read which is based on the historic interactions.
...
[Watch the Presentation & Demo Here](https://youtu.be/8IDTJWs90h0)
